...
export const DropDown = ({ label, options, dataObject, classNames }: TCustomDropDownProps): TemplateResult<1> => {
  return html`
    <div
      class="NAME-dropdown ${classNames?.container || ""}"
      data-value="${options[0].value}"
      data-object=${ifDefined(dataObject ? JSON.stringify(dataObject) : null)}
    >
      <button class="NAME-dropdown-btn ${classNames?.btn || ""}">${unsafeHTML(label)}</button>
      <div class="NAME-dropdown-content">
        ${repeat(
          ...
          ({ value, label }, index) =>
            html`
              <button class="NAME-dropdown-option ${classNames?.options?.[index] || ""}" data-value="${value}">
                ${unsafeHTML(label)}
              </button>
            `
        )}
